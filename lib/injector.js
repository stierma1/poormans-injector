...
var asyncFuncRegex = /^async\s+function\s*([a-zA-Z0-9_\$]*)\s*\(\{([a-zA-Z0-9_\$\,\s]*)\}/
var classRegex = /^class\s*([a-zA-Z0-9_\$]+)\s*\{\s*constructor\s*\(\{([a-zA-Z0-9_\$\,\s]*)\}/
var staticFuncRegex = /^\s*([a-zA-Z0-9_\$]+)\s*\(\{([a-zA-Z0-9_\$\,\s]*)\}/
...
class Injector{
  ...
  static parseParams(funcString){
    ...
    if(!match){
      match = asyncFuncRegex.exec(funcString);
    }
    if(!match){
      match = staticFuncRegex.exec(funcString);
    }
    if(match){
